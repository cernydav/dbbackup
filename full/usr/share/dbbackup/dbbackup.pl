...
# 0 2 * * * /usr/bin/python /usr/share/dbbackup/dbbackup.py
#
# This file should go to /usr/share/dbbackup/dbbackup.py
#
##########################################################
...
config.read("/etc/dbbbackup/settings.cnf")

# backup path
BACKUP_PATH =  config.get('settings', 'backup_path', '/var/backups/mysql/') 
# backup life time span after this time span folders with data are deleted
BACKUP_LIFE_IN_DAYS = config.get('settings', 'life_span', '7') # days
# Folder Naming Application Settings
DATETIME =  config.get('settings', 'datetime_format', '%Y-%m-%d') 
